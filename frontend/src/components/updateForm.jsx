...
function UpdateForm({ post, postId, handlePostCreated }) {
  ...
  useEffect(() => {
    if (post) {
      setPostObj({
        ...
        cover: post.cover || "",
        date: post.date ? new Date(post.date).toISOString().split('T')[0] : "", // Format date to YYYY-MM-DD
      });
    }
  }, [post]);
  ...
  function handleSubmit(e) {
    e.preventDefault();
    // Assuming postId is passed as a prop to UpdateForm

    try {
      axios.put(`http://localhost:3000/posts/${postId}`, postObj)
        .then((res) => {
